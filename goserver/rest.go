...
var (
	temp = promauto.NewGaugeVec(prometheus.GaugeOpts{
		Name: "temperature",
		Help: "Temperature of the sensor",
	}, []string{"id"},
	)
)
...
func mainGin(wg *sync.WaitGroup, settings *settings) {
	defer wg.Done()
...
func (e *Env) serialPost(c *gin.Context) {
	...
	if err := c.BindJSON(&line); err == nil {
		...
		if match != nil {
			...
			//PrintCol(nodes)
			temp.With(prometheus.Labels{"id": strconv.FormatInt(node.Id, 10)}).Set(node.Temp)
		}
